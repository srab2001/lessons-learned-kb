...
description: "Create a journey map documenting the end-to-end experience of a user or practitioner accomplishing a goal. Works for UX research (user journeys, service interactions) and technical documentation (developer onboarding flows, API integration journeys)."
inputs:
  - name: persona
...
### Example: Journey Map Summary Table (UX)
...
### Example: Journey Map Summary Table (Technical — Developer Onboarding)

```markdown
# Journey Map: New Engineer - First Independent Contribution

| Phase | Touchpoints | Emotions | Key Pain Points | Opportunities |
|-------|-------------|----------|-----------------|---------------|
| Orientation | README, AGENTS.md | 😐 Curious | Unclear where to start | Add "Next Steps" section |
| Setup | Clone, npm install | 😟 Frustrated | Missing env vars, no docs | Document setup prerequisites |
| First task | GitHub Issues, codebase | 😐 Uncertain | Hard to find right files | Improve code navigation docs |
| First PR | Review checklist, CI | 😟 → 😐 | Long review cycle | Add pre-PR checklist |
| Merge | Main branch | 😊 Accomplished | None | Celebrate first contribution |
```

This skill works equally well for mapping technical workflows — developer journeys, CLI usage flows, API integration paths, or any multi-step process where experience quality matters.
