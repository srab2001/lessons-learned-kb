...
description: "Create a service blueprint documenting frontstage and backstage processes that deliver a service experience. Works for government services, internal team workflows, and technical architecture (CLI/API systems, CI/CD pipelines, developer tooling)."
inputs:
...
### Example: Blueprint Summary (Application Submission — Government Service)
...
### Example: Blueprint Summary (Technical — CI/CD Pipeline)

```markdown
# Service Blueprint: Developer Push to Production Deployment

| Step | Push code | Open PR | Merge | Deploy | Verify |
|------|-----------|---------|-------|--------|--------|
| **Evidence** | Push confirmation | PR page | Merge button | Deploy log | Monitoring dashboard |
| **Developer** | git push | Reviews CI results | Approves + merges | Monitors pipeline | Checks metrics |
| ═══ INTERACTION ═══ |
| **Frontstage** | Branch created | PR checks displayed | Merge commit | Deployment triggered | Status page updated |
| ═══ VISIBILITY ═══ |
| **Backstage** | - | Lint, test, build jobs | Merge validation | Canary deploy, smoke test | Health checks |
| ═══ INTERNAL ═══ |
| **Support** | Git server | CI runners | Code review bot | Deploy orchestrator | APM, alerting |

**Failure Points:**
- ⚠️ CI flakiness causes false failures
- ⚠️ Canary traffic routing misconfiguration

**Wait Time:**
- ⏱️ PR open → first review: varies (no SLA enforced)
```

This skill works equally well for technical architecture documentation — CLI tool flows, API lifecycles, CI/CD pipelines, or any system where understanding the full stack of supporting processes clarifies where failures and bottlenecks occur.
